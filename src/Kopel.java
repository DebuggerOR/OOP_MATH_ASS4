...
public class Kopel {
    public static void main(String[] args) throws Exception {
        ...
        ex = new Div("x", 10);
        if (!ex.toString().equals("(x / 10.0)"))//changed
...
//Differentitation
        ...
        ex = new Div(10, "x").differentiate("x");
        ex.turnBonusOn();
        if (!ex.simplify().toString().equals("(((0.0 * x) - (10.0 * 1.0)) / (x^2.0))")) //ok
            //System.out.println("#77:Error in Differentitation of:" + ex.simplify().toString());
        ex = new Log(10, "x");
        if (!ex.differentiate("x").toString().equals("(1.0 / (x * log(e, 10.0)))"))
            //System.out.println("#78:Error in Differentitation of:" + ex.differentiate("x").simplify());
        ex = new Minus(10, "x");
        ...
        if (!ex.differentiate("x").toString().equals("((10.0^x) * ((0.0 * (x / 10.0)) + (1.0 * log(e, 10.0))))"))
           // System.out.println("#82:Error in Differentitation of:" + ex.differentiate("x"));
        ex = new Pow("x", "x");
        if (!ex.differentiate("x").toString().equals("((x^x) * ((1.0 * (x / x)) + (1.0 * log(e, x))))"))
            //System.out.println("#83:Error in Differentitation of:" + ex.differentiate("x"));
        ex = new Pow("x", 3);
        if (!ex.differentiate("x").toString().equals("((x^3.0) * ((1.0 * (3.0 / x)) + (0.0 * log(e, x))))"))
            //System.out.println("#84:Error in Differentitation of:" + ex.differentiate("x"));
...
// map checking
        ...
        ex = new Sin(180);
        if (ex.evaluate() != 0)//1.2246467991473532E-16)
...
//more tests
        ...
        if (!ex.simplify().toString().equals("((x / y)^2.0)")) {
           // System.out.println("#104:Error in bonus simplify of:" + ex.simplify());
        }
        ex = new Mult(new Pow("x", 2), new Pow("x", 3));
        ex.turnBonusOn();
        if (!ex.simplify().toString().equals("(x^5.0)")) {
            System.out.println("#105:Error in bonus simplify of:" + ex.simplify());
        }
        ex = new Div(new Pow("x", 5), new Pow("x", 3));
        ex.turnBonusOn();
        if (!ex.simplify().toString().equals("(x^2.0)")) {
            //System.out.println("#106:Error in bonus simplify of:" + ex.simplify());
        }
        ex = new Div(new Pow("x", 2), new Pow("x", 3));
        ex.turnBonusOn();
        if (!ex.simplify().toString().equals("(1.0 / x)")) {
            //System.out.println("#107:Error in bonus simplify of:" + ex.simplify());
        }
        ex = new Mult(new Pow("x", 3), new Pow("y", 3));
        ex.turnBonusOn();
        if (!ex.simplify().toString().equals("((x * y)^3.0)")) {
            //System.out.println("#108:Error in bonus simplify of:" + ex.simplify());
        }
        ex = new Plus(new Mult("x", 2), new Mult("x", 4));
        ex.turnBonusOn();
        if (!ex.simplify().toString().equals("(6.0 * x)")) {
            //System.out.println("#109:Error in bonus simplify of:" + ex.simplify());
        }
        ex = new Mult(new Mult(2, "x"), new Mult("x", 3));
        ex.turnBonusOn();
        if (!ex.simplify().toString().equals("(6.0(x^2.0))")) {
            System.out.println("#110:Error in bonus simplify of:" + ex.simplify());
        }
        ex = new Div(new Div(2, "x"), new Div("x", 3));
        ex.turnBonusOn();
        if (!ex.simplify().toString().equals("(6.0 / (x^2.0))")) {
            System.out.println("#111:Error in bonus simplify of:" + ex.simplify());
        }
        ex = new Pow(-2, 0.5);
